...
ratings. Καταλήξαμε με ένα αρχείο (Processed_Movie.tab) που έχει μόνο 26.691
...
#align(center)[
  #grid(
    ...
    column-gutter: 20pt,
    row-gutter: 4pt,
    image("images/AssociationItemsets.png", width: 80%),
    image("images/AssociationRules.png", width: 123%),
    text(style: "italic", size: 10pt)[Frequent Itemsets],
    text(style: "italic", size: 10pt)[Association Rules]
  )
]

#par[
  Το Frequent Itemsets εντοπίζει τους πιο συχνούς συνδυασμούς όλων των genres
  και των rating_class μεταξύ τους. Το Associaton Rules χρησιμοποιεί τα δεδομένα
  που βρίσκει το Frequent Itemsets και συνδυάζει τα genres και rating_class
  μεταξύ τους για να υπολογίσει το lift, που δείχνει αν μια ταινία σχετίζεται με
  μια άλλη επειδή έχει υψηλές αξιολογήσεις ή επειδή ταιριάζει με τις υπόλοιπες
  ταινίες που έχουν παρακολουθήσει οι χρήστες. Αν το lift είναι 1, δεν υπάρχει
  σχέση μεταξύ των στοιχείων του itemset, αν είναι μικρότερο του 1 μειώνεται και
  αν είναι μεγαλύτερο αυξάνεται.
]
...
Η ανάλυση του dataset μας έδωσε πληροφορίες για της ταινίες και βρήκε ενδιαφέρον
...
recommendation system (ειδικά το Association Rules) και προσφέρουν μια ενδιαφέρον
